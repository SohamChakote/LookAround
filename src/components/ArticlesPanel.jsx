...
export default function ArticlesPanel() {
  ...
  const [loadedTitles]                          = useState(new Set());
  ...
  const loadMoreFromWikipedia = useCallback(async () => {
    ...
    try {
      ...
      const summaries = await Promise.all(titles.map(t => fetchWikiSummary(t)));
      const newCards = summaries
        .filter(s => s !== null)
        .filter(s => !loadedTitles.has(s.title))
        .map(s => { loadedTitles.add(s.title); return wikiArticleToCard(s); });
      setWikiArticles(prev => [...prev, ...newCards]);
      if (newCards.length === 0) setLoadMoreError('No new articles found. Try again.');
    } catch (err) {
      setLoadMoreError(`Failed to load: ${err.message}`);
    } finally {
  ...
  return (
    <section className="panel articles-panel">
      <div className="panel-heading">
        <p className="eyebrow">Vancouver</p>
        <h1>Discover</h1>
        <p className="subtitle">Stories, history, and facts about the city.</p>
      </div>
      ...
      <div className="article-category-filter">
        {CATEGORIES.map(cat => (
          <button
            key={cat}
            className={`article-filter-btn${selectedCategory === cat ? ' active' : ''}`}
